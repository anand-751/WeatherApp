...
int main() {
    ...
    json weatherData = fetchWeather(apiKey, city);

    if (!weatherData.empty()) {
        std::cout << "Weather for " << city << ":" << std::endl;
        std::cout << std::setw(20) << std::left << "Description:" << weatherData["weather"][0]["description"] << std::endl;
        double temperatureCelsius = weatherData["main"]["temp"].get<double>() - 273.15;
        std::cout << std::setw(20) << std::left << "Temperature :" << std::fixed << std::setprecision(2) << temperatureCelsius << "(°C)" << std::endl;
        std::cout << std::setw(20) << std::left << "Humidity :" << weatherData["main"]["humidity"]<< "(%)" << std::endl;
        std::cout << std::setw(20) << std::left << "Wind Speed :" << weatherData["wind"]["speed"] <<"(m/s)"<< std::endl;

        time_t date = weatherData["dt"];
        std::cout << std::setw(20) << std::left << "Date:" << std::put_time(std::localtime(&date), "%Y-%m-%d") << std::endl;
        std::cout << std::setw(20) << std::left << "Day:" << weatherData["sys"]["country"] << std::endl;
        time_t sunrise = weatherData["sys"]["sunrise"];
